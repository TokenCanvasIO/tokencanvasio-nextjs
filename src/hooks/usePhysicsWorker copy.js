...
export const usePhysicsWorker = (coinData, timeframe, bubbleSpeed, isDesktop) => {
    ...
    // THIS IS THE FIX:
    // This new logic correctly limits the number of bubbles WHILE PRESERVING
    // ALL original data properties, including the crucial 'tokenId'.
    const processedCoinData = useMemo(() => {
        const limit = isDesktop ? bubbleCountLimits.desktop : bubbleCountLimits.mobile;

        // Take a slice of the original data up to the limit
        const limitedData = coinData.slice(0, limit);
        
        // Now, map over the final, limited list to add the numericId
    ...
    const api = useMemo(() => ({
        ...
        removeBodies: (bodyIds) => workerRef.current?.postMessage({ type: 'REMOVE_BODIES', payload: { ids: bodyIds } }),
